...
See all invoices ranked by risk (who is most likely to not pay)
Send early payment discount offers to customers to get paid faster
Track whether customers accepted or ignored the offer
TECH STACK
...
Navy 
#1A3C5E — headers, sidebar, primary buttons
Teal 
#0F766E — collection actions, overdue alerts
Purple 
#7C3AED — early payment offer actions and badges
Background 
#F8FAFC
...
"Send Offer" button click: Opens a small inline form that drops down BELOW that specific table row (not a modal). The form contains:
...
"Withdraw" button click: Calls POST /api/offers/:id/withdraw immediately (no confirmation needed). Refreshes that row. Shows toast "Offer withdrawn."
...
Below the feed, show one summary line: "Total saved by early payment this session: $X,XXX"

BUYER ACCEPTANCE SIMULATION (for demo purposes)

In the offer activity feed, each active offer row has a small [Simulate Acceptance] button. Clicking it calls POST /api/offers/:id/accept. Refreshes both the table and the feed. Shows toast: "✅ [Customer] accepted the offer! Invoice marked paid."
...
No campaign builder
DONE MEANS:
npm start (or equivalent) runs without errors
The stats bar shows real numbers from seeded data
The invoice table loads all 15 invoices, sorted red-first
Clicking "Send Offer" on an overdue invoice creates an offer and shows the purple badge
Clicking "Simulate Acceptance" marks the invoice paid and updates the feed
Withdrawing an offer removes the badge from the row
The stats bar updates after each action
